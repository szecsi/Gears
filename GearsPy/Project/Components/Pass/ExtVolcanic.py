...
float noise( in vec3 x )
{
	...
	vec2 rg = texture2D( iChannel0, (uv+ 0.5)/256.0 * vec2(1, 1), -100.0 ).yx;
...
float noise( in vec2 x )
{
	...
	return texture2D( iChannel0, (uv+118.4)/256.0 * vec2(1, 1), -100.0 ).x;
}
...
vec3 path( float stime )
{
	return vec3( 16.0*cos(0.2+0.5*.1*stime*1.5), 1.5, 16.0*sin(0.1+0.5*0.11*stime*1.5) );
...
void main(  )
{
    ...
    // camera	
	float stime = 2.7+time;
	vec3 ro = path( stime+0.0 );
	vec3 ta = path( stime+1.6 );
	//ta.y *= 0.3 + 0.25*cos(0.11*stime);
	ta.y *= 0.35 + 0.25*sin(0.09*stime);
	float roll = 0.3*sin(1.0+0.07*stime);
