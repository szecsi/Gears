...
class Crossing(Base) : 
    ...
    def applyWithArgs(
        ...
        travelLength = math.fabs(sequence.field_width_um * c) + math.fabs(sequence.field_height_um * s) + shapeLength_um
        spass.duration = int(travelLength / velocity / sequence.getFrameInterval_s() + 1.0 )
        if extendStimulusDurationToCrossingDuration:
            stimulus.duration = max(stimulus.duration, spass.duration)

        spass.setShaderVector( name = functionName + '_startPosition', x=-travelLength*0.5*c - offset_um*s, y=-travelLength*0.5*s + offset_um*c )
