...
from PyQt5.QtWidgets import (QWidget, QOpenGLWidget, QToolTip, QPushButton, QMessageBox, QApplication, QTreeWidget, QTreeWidgetItem, QGridLayout, QGroupBox, QTabWidget, QDoubleSpinBox, QLabel, QRadioButton, QButtonGroup, QScrollBar, QSlider, QTextEdit)
...
##        self.setPalette(pal);
        ...
        toneMappingRadioGroup = QGroupBox('Intensity mapping', self.calibrationPanel)
        ...
        self.saveToneMapSettingsButton = QPushButton('Save intensity mapping')
        ...
        self.optionsTabber.addTab(self.calibrationPanel, 'Intensity')
        ...
        hintLabel = QLabel('''
                ...
                <p><b>F</b> : show/hide FPS</p>
                <p><b>S</b> or <b>-&gt;</b> : skip to next</p>
                <p><b>P</b> : pause </p>
                <p><b>Space</b> : show stim ctrls  </p>
                ''')
...
#        self.setWindowIcon(QIcon('web.png'))
    ...
    def initSequence(self, browserWindow, path):
        ...
        if self.calibrating :
            ...
            print('''
    stim.setMeasuredDynamics( {dmin}, {dmax}, {dmean}, {dvar})
                '''.format(
                        dmin  = stim.measuredDynamicRangeMin,
                        dmax  = stim.measuredDynamicRangeMax,
                        dmean = stim.measuredDynamicRangeMean,
                        dvar  = stim.measuredDynamicRangeVar, )
    ...
    def start(self, browserWindow, path):
        ...
        #self.calibHistoMinValue.setValue(self.exp.)
    ...
    def launchCalibration(self):
        ...
        timestr = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        pathInCalibs = self.path.replace('Sequences', 'Calibration', 1)
        ...
        print('#Calibration at ' + timestr, file=self.calibrationSettingsFile) 
    ...
    def stimulusChanged(self):
        ...
        elif stim.dynamicRangeMin == 0 and stim.dynamicRangeMax == 1 :
        ...
        self.calibToneMinValue. setValue( stim.dynamicRangeMin )
        self.calibToneMaxValue. setValue( stim.dynamicRangeMax )
        self.calibToneMeanValue.setValue( stim.dynamicRangeMean)
        self.calibToneVarValue. setValue( stim.dynamicRangeVar )

        self.calibMeasuredMinValue.setText(  "{v:.2f}".format(v=stim.measuredDynamicRangeMin ));
        self.calibMeasuredMaxValue.setText(  "{v:.2f}".format(v=stim.measuredDynamicRangeMax ));
        self.calibMeasuredMeanValue.setText( "{v:.2f}".format(v=stim.measuredDynamicRangeMean));
        self.calibMeasuredVarValue.setText(  "{v:.2f}".format(v=stim.measuredDynamicRangeVar ));
    ...
    def onStimZoomSlider(self, value) :
        ...
        print('setting' + str(self.midFrame - value * self.mousePos))
    ...
    def applyMeasuredToneMin(self, d):
        stim = gears.getSelectedStimulus().getPythonObject()
        stim.dynamicRangeMin = stim.measuredDynamicRangeMin 
        self.calibToneMinValue.setValue(stim.dynamicRangeMin)
        self.update()

    def applyMeasuredToneMax(self, d):
        stim = gears.getSelectedStimulus().getPythonObject()
        stim.dynamicRangeMax = stim.measuredDynamicRangeMax 
        self.calibToneMaxValue.setValue(stim.dynamicRangeMax)
        self.update()

    def applyMeasuredToneMean(self, d):
        stim = gears.getSelectedStimulus().getPythonObject()
        stim.dynamicRangeMean = stim.measuredDynamicRangeMean 
        self.calibToneMeanValue.setValue(stim.dynamicRangeMean)
        self.update()
    ...
    def applyMeasuredToneVar(self, d):
        stim = gears.getSelectedStimulus().getPythonObject()
        stim.dynamicRangeVar = stim.measuredDynamicRangeVar 
        self.calibToneVarValue.setValue(stim.dynamicRangeVar)
        self.update()
    ...
    def saveToneMapSettings(self) :
        ...
        if self.toneMappingRadioErf.isChecked() :
            print('    stim.setToneMappingErf({dmean},{dvar})'.format(dmean=self.calibToneMeanValue.value(), dvar=self.calibToneVarValue.value()), file=toneMapSettingsFile) 
            stim.setToneMappingErf(self.calibToneMeanValue.value(), self.calibToneVarValue.value())
        elif self.toneMappingRadioNone.isChecked() :
            print('    pass', file=toneMapSettingsFile) 
            stim.setToneMappingLinear(0, 1)
        else :
            print('    stim.setToneMappingLinear({dmin},{dmax})'.format(dmin=self.calibToneMinValue.value(), dmax=self.calibToneMaxValue.value()), file=toneMapSettingsFile) 
            stim.setToneMappingLinear(self.calibToneMinValue.value(), self.calibToneMaxValue.value())
