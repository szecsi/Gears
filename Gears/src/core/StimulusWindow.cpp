...
LRESULT StimulusWindow::winProc(HWND   hwnd, UINT   uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
	{
		...
		stimulus->executeCallbacks<Gears::Event::KeyPressed>(Gears::Event::KeyPressed::create(uMsg, wParam, lParam));
		break;
	}
	case WM_MOUSEMOVE:
	{
		Stimulus::CP stimulus = sequenceRenderer->getCurrentStimulus();
		stimulus->executeCallbacks<Gears::Event::MouseMove>(Gears::Event::MouseMove::create(uMsg, wParam, lParam));
		break;
	}
	case WM_MOUSEWHEEL:
	{
