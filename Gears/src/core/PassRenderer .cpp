...
void PassRenderer::renderPass(int skippedFrames)
{
	...
	if( stimulusRenderer->hasSpatialFilter() && stimulus->spatialFilter->useFft )
		stimulusGeneratorShader->bindUniformFloat2("patternSizeOnRetina", stimulus->sequence->getSpatialKernelWidth_um(), stimulus->sequence->getSpatialKernelHeight_um());
	else
	...
	if(stimulus->spatialFilter != nullptr)
		stimulusGeneratorShader->bindUniformBool("swizzleForFft", stimulus->spatialFilter->useFft);
	else
		stimulusGeneratorShader->bindUniformBool("swizzleForFft", false);
	...
	if(stimulus->temporalProcessingStateCount > 0)
	{
		...
		stimulusGeneratorShader->bindUniformMatrix("stateTransitionMatrix", (float*)stimulus->temporalProcessingStateTransitionMatrix, 1 /*TODO 4 if 7 states*/);
	}
	...
	{
		stimulusGeneratorShader->bindUniformFloat("dynamicRangeMin", stimulus->dynamicRangeMin);
		stimulusGeneratorShader->bindUniformFloat("dynamicRangeMax", stimulus->dynamicRangeMax);
		if(stimulus->erfToneMapping)
		{
			stimulusGeneratorShader->bindUniformFloat("dynamicRangeMean",stimulus->dynamicRangeMean);
			stimulusGeneratorShader->bindUniformFloat("dynamicRangeVar", stimulus->dynamicRangeVar);
		}
		else
		{
			stimulusGeneratorShader->bindUniformFloat("dynamicRangeMean",0.f);
			stimulusGeneratorShader->bindUniformFloat("dynamicRangeVar", -1.f);
		}
		stimulusGeneratorShader->bindUniformBool("calibrating", sequenceRenderer->calibrating);
