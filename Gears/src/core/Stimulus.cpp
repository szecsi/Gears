...
boost::python::object Stimulus::getPythonObject() const
{
	return pythonObject;
}
	...
	)const
{
	...
	for (int i = 0; i < boost::python::len(histogramList) ; i++){
		const_cast<Stimulus*>(this)->measuredHistogram.push_back(boost::python::extract<float>(histogramList[i]));
	}
	return boost::python::object();
}
...
boost::python::object Stimulus::setLtiMatrix(boost::python::object mList)
{
	...
	else if(length=64)
	{
		temporalProcessingStateTransitionMatrix[0] = Gears::Math::float4x4(
			...
			;
		temporalProcessingStateTransitionMatrix[1] = Gears::Math::float4x4(
			 extract<float>(l[4]),   extract<float>(l[5]),   extract<float>(l[6]),   extract<float>(l[7]),
			 extract<float>(l[12]),  extract<float>(l[13]),  extract<float>(l[14]),  extract<float>(l[15]),
			...
			;
		temporalProcessingStateTransitionMatrix[2] = Gears::Math::float4x4(
			...
			;
		temporalProcessingStateTransitionMatrix[3] = Gears::Math::float4x4(
...
boost::python::object Stimulus::setLtiImpulseResponse(boost::python::object mList, uint nStates)
{
	fullScreenTemporalFiltering = true;
	doesToneMappingInStimulusGenerator = false;
