...
namespace Gears
{
	...
	public:
		...
		/// @param defaultValue the value that should be returned if the specified field does not exist
		/// @return the value of the table field
		Gears::Math::float3 getFloat3(const std::string& key, Gears::Math::float3 defaultValue = Gears::Math::float3(1, 1, 1))
		{
			using namespace boost::python;
			if(!dictionary.has_key(key))
				return defaultValue;
		...
		/// @param defaultValue the values that should be returned where the channel fields do not exist
		/// @return the value of the vactor
		Gears::Math::float3 asFloat3(Gears::Math::float3 defaultValue = Gears::Math::float3(1, 1, 1))
		{
			return Gears::Math::float3 (
		...
		template<typename T>
		boost::shared_ptr<T> get(const std::string& key, boost::shared_ptr<T> defaultValue)
		{
			using namespace boost::python;
			if(!dictionary.has_key(key))
				return defaultValue;
			object a = dictionary[key];
			extract<boost::shared_ptr<T>> gs(a);
			if(!gs.check()) {
				std::stringstream ss;
		...
		void forEach(std::string key, std::function<void(boost::python::object)> f)
		{
			using namespace boost::python;
			if(!dictionary.has_key(key))
				throwError(std::string("Required lua table entry '") + key + "' not found or is 'None'.");
