...
public:
	union{
		...
		float4swizzle<4, float4, 0, 1, 2, -1> xyz0;
		float4swizzle<4, float4, 0, 1, 2, -2> xyz1;
	...
	bool4 isinf() const
	{
		return bool4( !std::isfinite(x), !std::isfinite(y), !std::isfinite(y), !std::isfinite(w) );
	}
	...
	float4 quatConj() const
	{
		return float4(-x, -y, -z, w);
	}

	float4 quatInv() const
	{
		return quatConj() / lengthSquared();
	}

	float4 quatExp() const
	{
		float3 vv = xyz;
